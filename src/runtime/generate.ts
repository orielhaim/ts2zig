export function generateRuntime(): string {
  ...
  pub fn ReturnType(comptime func: anytype) type {
      return ReturnTypeOf(@TypeOf(func));
  }
  
  /// Extract the return type from a function *type* (not a value).
  pub fn ReturnTypeOf(comptime T: type) type {
      ...
      @compileError("ReturnTypeOf expects a function or function pointer, found '" ++ @typeName(T) ++ "'");
  }
  
  /// Extract the first parameter type from a function type.
  /// Used to resolve generic type T from callback (item: T) => U.
  pub fn FirstParamType(comptime T: type) type {
      const info = @typeInfo(T);
      if (info == .@"fn") {
      ...
      @compileError("FirstParamType expects a function, found '" ++ @typeName(T) ++ "'");
  }
  
  /// Extract the child element type from a std.ArrayList(T) type.
  /// Returns T given std.ArrayList(T).
  pub fn ArrayListChild(comptime T: type) type {
      const info = @typeInfo(T);
      if (info == .@"struct") {
